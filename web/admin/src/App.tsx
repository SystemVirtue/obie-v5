...
function SearchPanel() {
  ...
  const performSearch = async () => {
    ...
    try {
      const [ytSettled, r2Settled] = await Promise.allSettled([
        callKioskHandler({ action: 'search', query: trimmed }) as Promise<{ videos?: SearchResult[] }>,
        callKioskHandler({ action: 'search_r2', query: trimmed }) as Promise<{ videos?: SearchResult[] }>,
      ]);
  ...
  const handleAddToPlaylist = async () => {
    ...
    try {
      ...
      if (res?.error) throw new Error(res.error);
      if (!res.media_item_id) throw new Error('No media_item_id returned');
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <PanelHeader
        title="Search"
        subtitle="Search library and YouTube without consuming kiosk credits"
      />
      ...
      <div style={{ flex: 1, overflowY: 'scroll', padding: 24 }}>
        <div style={{ maxWidth: 480 }}>{errBlock}
          ...
          {'cloudflare_enabled' in local && (
            <SettingsRow label="Cloudflare R2 Search" desc="Include synced Cloudflare library results in kiosk search and playback">
              <Toggle checked={!!local.cloudflare_enabled} onChange={() => handleToggle('cloudflare_enabled' as keyof PlayerSettings)} />
            </SettingsRow>
          )}
          {'cloudflare_r2_public_url' in local && !!local.cloudflare_enabled && (
            <SettingsRow label="R2 Public URL" desc="Public base URL used for Cloudflare-hosted media playback">
              <input
                type="text"
                placeholder="https://pub-xxxxxxxx.r2.dev"
                value={local.cloudflare_r2_public_url ?? ''}
                onChange={e => set('cloudflare_r2_public_url' as keyof PlayerSettings, e.target.value)}
                style={{ width: 240, padding: '7px 10px', borderRadius: 9, background: '#111', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 12, outline: 'none' }}
              />
            </SettingsRow>
          )}
