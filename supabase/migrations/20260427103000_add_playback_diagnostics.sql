-- Playback diagnostics used by the player and admin console to distinguish
-- queued/loading state from confirmed playback and recent failure reasons.
...
ALTER TABLE public.player_status
  ...
  ADD COLUMN IF NOT EXISTS playback_error_code TEXT,
  ADD COLUMN IF NOT EXISTS playback_error_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS last_recovery_reason TEXT;

CREATE INDEX IF NOT EXISTS idx_player_status_playback_error_at
  ON public.player_status (playback_error_at DESC)
  WHERE playback_error_at IS NOT NULL;

-- Reset confirmation/error fields at each queue transition.
