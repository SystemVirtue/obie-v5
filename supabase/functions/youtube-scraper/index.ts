...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if ((type === 'auto' && playlistId) || type === 'playlist') {
          ...
          return new Response(
            JSON.stringify({ error: 'Could not detect video or playlist ID from URL' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
    ...
    return new Response(
      JSON.stringify({ videos, count: videos.length }),
...
async function fetchVideo(videoId: string, apiKey: string): Promise<YouTubeVideo | null> {
...
async function fetchPlaylist(playlistId: string, apiKey: string): Promise<YouTubeVideo[]> {
...
async function fetchVideosBatch(videoIds: string, apiKey: string): Promise<YouTubeVideo[]> {
...
function parseVideoItem(item: any): YouTubeVideo {
