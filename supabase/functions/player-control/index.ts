...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const body: PlayerStatusUpdate = await req.json();
    ...
    if (action === 'heartbeat') {
      const { error } = await supabase
        .rpc('player_heartbeat', { p_player_id: player_id });
    ...
    if (action === 'update' || action === 'ended' || action === 'skip') {
      ...
      if (action === 'skip' && state === 'idle') {
        ...
        return new Response(
          JSON.stringify({ success: true, skip_pending: true }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (action === 'ended' || state === 'idle') {
        ...
        return new Response(
          JSON.stringify({ success: true, next_item: nextItem?.[0] || null, action }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
